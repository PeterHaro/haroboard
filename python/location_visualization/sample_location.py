...
def get_countries(df, geojson_name, granularity=2):
    ...
    with open(geojson_name) as data_file:
...
def clean_location_data(sample_frac=0.05):
    with open('./../../public/data/peter_location_history/NotProcessedLocationHistory.json') as data_file:
    ...
    df = get_countries(df, './../../public/data/natural_earth/geojson/ne_50m_admin_0_countries.geojson')

    df_sample = sample(df, sample_frac)[['time', 'lat', 'lon', 'country']]

    final_json = df_sample.to_json(orient='records')
    with open('./../../public/data/peter_location_history/CleanedLocationHistory.json', 'w') as outfile:
